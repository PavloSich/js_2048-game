...
function render() {
    ...
    if (game.getStatus() === "win") msgWin.classList.remove("hidden");
    if (game.getStatus() === "lose") msgLose.classList.remove("hidden");
}
...
class Game {
    ...
    transpose() {
        ...
        for(let c = 0; c < 4; c++){
            res[c] = [];
            for(let r = 0; r < 4; r++)res[c][r] = this.state[r][c];
        }
        this.state = res;
    }
    updateStatus() {
        ...
        if (!this.hasMoves()) this.status = "lose";
    }
    hasMoves() {
